...
public class ConsoleGUI : MonoBehaviour, IConsoleUI
{
    ...
    public void Init()
    {
        buildIdText.text = Application.productName + " (" + Application.version + ")";
    }
    ...
    private string openKeys = String.Empty;
    private string rightKeys = "ABABAABB";
    private bool _openDebugConsole = false;

    public void ConsoleUpdate()
    {
        if (!_openDebugConsole)
        {
            if (Input.GetKeyDown(KeyCode.A)) openKeys += "A";
            if (Input.GetKeyDown(KeyCode.B)) openKeys += "B";
            if (rightKeys.StartsWith(openKeys))
            {
                if (rightKeys == openKeys)
                {
                    _openDebugConsole = true;
                    SetOpen(!IsOpen());
                }
            }
            else
            {
                openKeys = String.Empty;
            }
            return;
        }
