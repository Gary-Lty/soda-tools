    ...
    public override void ProcessFrame(Playable playable, FrameData info, object playerData)
    {
        ...
        for(int i=0;i<inputCount;i++)
        {
            ...
            if(input.triggered)
            {
                ...
                if (GameManager.SoundSystem == null)
                {
                ...
                switch (input.position)
                {
                    case SoundTimelineBehaviour.SoundPosition.None:
                        GameManager.SoundSystem.Play(input.sound);
                        break;
                    case SoundTimelineBehaviour.SoundPosition.Position:
                        ...
                        GameManager.SoundSystem.Play(input.sound, trackBinding.position);
                        break;
                    case SoundTimelineBehaviour.SoundPosition.Follow:
                        ...
                        GameManager.SoundSystem.Play(input.sound, trackBinding);
                        break;
                }
                Profiler.EndSample();    
            }
